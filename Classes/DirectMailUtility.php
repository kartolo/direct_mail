...
class DirectMailUtility {
	 ...
	 */
	static function fetchRecordsListValues(array $listArr, $table, $fields='uid,name,email') {
		$outListArr = array();
	 ...
	 */
	static function cleanPlainList(array $plainlist) {
		/**
	 ...
	 */
	static function getIdList($table, $pidList, $groupUid, $cat) {
		$addWhere = '';
...
//		$command = $table.'.uid';
	 ...
	 */
	static function getMailGroups($list, array $parsedGroups, $perms_clause) {
	 ...
	 */
	static function rearrangeCsvValues(array $lines, $fieldList) {
		$out = array();
	 ...
	 */
	static function rearrangePlainMails(array $plainMails) {
		$out = array();
	 ...
	 */
	static function getRecordOverlay($table, array $row, $sys_language_content, $OLmode = '') {
	 ...
	 */
	static function formatTable(array $tableLines, array $cellParams, $header, array $cellcmd = array(), $tableParams = 'class="table table-striped table-hover"') {
		reset($tableLines);
	 ...
	 */
	static function getRecordList(array $listArr, $table, $pageId, $editLinkFlag = 1, $sys_dmail_uid = 0) {
		$count = 0;
		$lines = array();
		$out = '';
	 ...
	 */
	static function substituteMarkerArray($content, array $markContentArray, $wrap = '', $uppercase = 0, $deleteUnused = 0) {
	 ...
	 */
	static public function implodeTSParams(array $p, $k = '') {
		$implodeParams = array();
		if (is_array($p)) {
			foreach ($p as $kb => $val) {
				if (is_array($val)) {
